---
name: wide-multipass
multipass_desc: "Review diff, working tree, or full repo for actionable bugs and regressions. Covers /diff-rev, /code-rev, and /wide-rev style requests."
description: "-"
when_to_use: "When reviewing diffs, working tree, or full repo for actionable bugs and regressions, or when the user mentions broad review, general review, or diff review."
allowed-tools:
  - Read
  - Grep
  - Glob
...
Review code changes for actionable bugs or regressions, then keep only findings that survive self-challenge.

## Scope

- Review the current diff by default, or the full repo, branch, commit, or path when scope is expanded.
- Focus on discrete bugs or regressions the author would likely fix if aware.
- Prefer code paths with real behavioral impact over broad quality commentary.

## Workflow

1. Read the relevant files and surrounding context once before forming findings.
2. Generate candidate bugs only when there is concrete behavioral impact.
3. Self-challenge each candidate:
   - trace the full execution path
   - verify the operations touch the same resource or invariant
   - construct a concrete failure scenario
   - check whether the observed difference is intentional
4. Drop any finding that fails self-challenge instead of downgrading it.

## Findings Standard

- Prioritize issues that affect correctness, security, performance, or maintainability in ways the author would likely fix.
- Keep findings discrete, actionable, and brief.
- Ignore trivial style, linter or typechecker catches, broad cleanup advice, and pre-existing bugs unless full-repo scope is active.
- Use severity labels honestly: `[P0]` drop everything, `[P1]` urgent, `[P2]` normal, `[P3]` low.

## Guardrails

- Prefer no findings over weak speculation. A finding is weak when it lacks a concrete failure scenario, relies on "might" or "could" without tracing the execution path, or survives only by assuming unusual runtime conditions. Drop these instead of downgrading severity.
- Emit raw JSON only when JSON was requested, because prose wrappers break downstream parsers.
- Keep line ranges tight and evidence concrete.
...
- Overall correctness: (correct | incorrect | uncertain)
- Not reviewed:
